...
GUI is not shared, you will have to code your own.
99% of the overhead is waiting for URL requests, therefore I coded this code to be userfriendly rather than fast.
...
class overseer: the main user UI class. This is class handle gambling/monitoring user input. This is the class you wish to insert in your GUI later on (GUI not shared on this repo).

class monitor: monitors matches. A new instance of this class is created a lives in its own thread for each match you wish to monitor or gamble on.
This class also performs the URL requests (the scrap functions in this repo are placeholder - because its implementation will depends on the broker you wish to monitor, the website version etc.)
This class handle the database in the form of a dictionnary (because 99% of the overhead is URL request and ping time, there is no need for anything faster, just make it simple and user friendly rather).
...
If you are a new user, these could be usefull to train on:   
